...
void * best_fit_allocator(unsigned int size) {
	...
	do {
		if (size <= current_region->size && (!best_fit_region || best_fit_region->size > current_region->size)) {
	...
	if (best_fit_region) {
		...
		tmp->pointer = best_fit_region->pointer;
		tmp->size = size;
		...
		if (best_fit_region->size == size) {
			if (best_fit_region == free_regions) {
				free_regions = free_regions->next;
				if (free_regions != NULL) {
					free_regions->prev = NULL;
				}
			}else{
				if (best_fit_region->prev != NULL) {
					best_fit_region->prev->next = best_fit_region->next;
				}
				if (best_fit_region->next != NULL) {
					best_fit_region->next->prev = best_fit_region->prev;
				}
			}
			free(best_fit_region);
		}else{
			best_fit_region->pointer += size;
			best_fit_region->size -= size;
		}
		return tmp->pointer;
	}else{
		return NULL;
