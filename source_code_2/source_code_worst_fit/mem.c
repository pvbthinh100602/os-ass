...
void * worst_fit_allocator(unsigned int size) {
	...
	do {
		if (size <= current_region->size && (!worst_fit_region || worst_fit_region->size < current_region->size)) {
	...
	if (worst_fit_region) {
		...
		tmp->pointer = worst_fit_region->pointer;
		tmp->size = size;
		...
		if (worst_fit_region->size == size) {
			if (worst_fit_region == free_regions) {
				free_regions = free_regions->next;
				if (free_regions != NULL) {
					free_regions->prev = NULL;
				}
			}else{
				if (worst_fit_region->prev != NULL) {
					worst_fit_region->prev->next = worst_fit_region->next;
				}
				if (worst_fit_region->next != NULL) {
					worst_fit_region->next->prev = worst_fit_region->prev;
				}
			}
			free(worst_fit_region);
		}else{
			worst_fit_region->pointer += size;
			worst_fit_region->size -= size;
		}
		return tmp->pointer;
	}else{
		return NULL;
